...
function createLogger(level: LogLevel) {
  ...
  return function (message: string) {
    ...
    const messages = {
      info: {
        text: `[INFO:${timestamp}] ${message}`,
        color: COLORS.blue,
      },
      warn: {
        text: `[WARNING:${timestamp}] ${message}`,
        color: COLORS.orange,
      },
      error: {
        text: `[ERROR:${timestamp}] ${message}`,
        color: COLORS.red,
      },
    };

    return console.log(`%c${messages[level].text}`, messages[level].color);*/
    ...
    console.log(
      `%c[${logPrefix[level]}: ${timestamp}] ${message}`,
      logColor[level],
    );
  };
}
